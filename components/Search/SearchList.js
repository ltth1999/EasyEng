...
export default class SearchHome extends Component {
    ...
    requestAPISearch= _.debounce(()=>{
        ...
        const apiURL="http://my-json-server.typicode.com/mizukage1999/SearchDB/search"
            ...
            <ListItem avatar >                                               
                <Left>
                    <Icon name="ios-search" style={{fontSize: 20, color: '#0208c4'}} 
                            onPress={() => navigate('SearchDetails', item)}  />
                </Left>

                <Body>
                    <TouchableOpacity onPress={() => navigate('SearchDetails', item)} >
                        <Text style={{color:'#041700', fontSize:14}} >{item.nameEn} {item.IPA}</Text> 
    ...
    handleSearch =(text)=>{
        ...
        const data= _.filter(this.state.fullData, search=>{ //lọc dữ liệu vào mảng fullData
            if(search.nameEn.includes(formatedQuery)){
                return true
            }
            return false
        })
    ...
    render() {
        ...
        <Container>
            <SearchBar
                ...
                placeholder="Tra từ Anh sang Việt"
